...
- `workedExample` — optional narration + code (runnable via Pyodide)
- `interactiveDemo` — sliders + `codeTemplate` with `{{sliderId}}` placeholders; re-runs on change
- `checkYourself` — interactive prompt / reveal pairs
- `callout` — `tip` | `warning` | `interview`
- `mermaid` — diagram block rendered via `MermaidDiagram`

AI/ML chapters also merge interactive enrichments from `src/lib/data/aiLearnChapterEnrichment.js` (generated by `scripts/generate-ai-learn-chapter-enrichment.mjs`).

`wrapUp.nextSteps` entries may be strings or `{ label, href, exerciseId? }`. When `exerciseId` is set, links open `#ml-practice-lab/<exerciseId>` on the lesson page.
