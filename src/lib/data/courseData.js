...
import { rawLldAdvancedModules } from './lldAdvancedLabs.js';
import { rawLldHotPathModules } from './lldHotPathLabs.js';
import { rawLowLevelModules as rawCoreLowLevelModules } from './lldCoreCourseData.js';
...
export const siteOverview = {
  ...
  "learningPaths": [
    ...
    },
    {
      ...
      "summary": "Master OOP foundations, SOLID, design patterns, hot-path cache and rate-limit prompts, and machine-coding classics, then rehearse with interactive labs and timed practice.",
      "startModule": "lld-foundations",
      "focus": ["SOLID & modeling", "Design patterns", "Caches, rate limits, and machine-coding labs"]
    },
    {
...
const lldModules = [...rawCoreLowLevelModules, ...rawLldLearningModules, ...rawLldAdvancedModules, ...rawLldHotPathModules];
const dsaModules = [
...
const flowDefinitions = [
  ...
  },
  {
    ...
    summary: 'Learn object modeling, SOLID, design patterns, hot-path cache and rate-limit labs, and classic machine-coding drills, then rehearse with interactive practice.',
    description: 'Use this flow to build durable OO design judgment and to prepare for interviews focused on classes, interfaces, caches, rate limiters, workflows, patterns, testing seams, and evolving requirements.',
    ...
    cadence: 'Work foundations and SOLID first, then patterns and project labs, then hot-path labs and machine-coding classics with timed practice.',
    ...
    focusAreas: [
      'Prompt framing, invariants, SOLID, and responsibility assignment',
      'Design patterns, interactive topic labs, hot-path cache and rate-limit labs, and object-design project labs',
      'Machine-coding classics, testing seams, concurrency, and scale follow-ups'
    ],
...
export function getLessonPracticeSteps(lesson) {
  ...
  const isLowLevelDesign = lesson.flowSlug === 'low-level-design';
  if (isLowLevelDesign) {
    const openingStructure = ['Version-one scope', 'Functional requirements vs deferred follow-ups', 'Invariants and edge cases', 'Explicit non-goals'];
    const designStructure = ['Core classes and responsibilities', 'Models, services, and ports layout', 'State transitions and validation', 'Running happy-path plan'];
    const reviewStructure = ['Evaluator checklist: runs and OOP cleanliness', 'Concurrency and shared state', 'Edge cases and acceptance checks', 'Extensibility and next refactor'];

    return [
      {
        id: 'opening',
        kind: 'interview',
        title: 'Frame the LLD prompt',
        objective: 'Time-box the first few minutes around scope, functional requirements, invariants, and deferred follow-ups before naming a large class hierarchy.',
        prompt: `You are starting a machine-coding / LLD interview for "${lesson.title}". Spend 5-10 minutes capturing version-one scope, functional requirements, critical use cases, invariants, and what you are intentionally postponing or treating as a non-goal for now.`,
        guardrails: [
          prompts[0] ?? `Start by framing the highest-signal LLD question for ${lesson.title}.`,
          'Separate what must work in version one from follow-ups you will defer intentionally.',
          ...sectionHeadings.slice(0, 2),
          ...checklist.slice(0, 1)
        ],
        structure: openingStructure,
        template: buildLldPracticeTemplate(lesson.title, openingStructure, currentPromptLabel(prompts[0], lesson.title))
      },
      {
        id: 'design',
        kind: 'design',
        title: 'Object model and APIs',
        objective: 'Sketch the smallest coherent object model that can ship a running happy path first: classes, package layout, state rules, and the seams that absorb change.',
        prompt: `Design the object model for "${lesson.title}". Name core classes/responsibilities, the models/services/ports layout, the public APIs, state transitions, validation ownership, and how you will get a runnable happy path working before polishing extensibility.`,
        guardrails: [
          ...sectionHeadings,
          'Prefer a working happy path before speculative abstractions.',
          prompts[1] ?? `Identify the critical class boundary and one invariant for ${lesson.title}.`,
          ...checklist.slice(1, 3)
        ],
        structure: designStructure,
        template: buildLldPracticeTemplate(lesson.title, designStructure, currentPromptLabel(prompts[1], lesson.title))
      },
      {
        id: 'tradeoffs',
        kind: 'review',
        title: 'Tests, concurrency, and follow-ups',
        objective: 'Close like a strong LLD candidate: run/readability checklist, test seams, concurrency risks, follow-up absorption, and deferred cleanup.',
        prompt: `Review your LLD answer for "${lesson.title}". Check whether it runs, whether the OOP boundaries stay clean, which edge cases and concurrency risks matter, how follow-ups land, and the next refactor you would do with more time.`,
        guardrails: [
          prompts[2] ?? `Name the most important extensibility or concurrency trade-off for ${lesson.title}.`,
          ...pitfalls,
          `Use these evaluator checks as a final pass: runs or demos cleanly; no god class; key edges handled; concurrency risk named; extensibility story is believable; ${checklist.join('; ')}`
        ],
        structure: reviewStructure,
        template: buildLldPracticeTemplate(lesson.title, reviewStructure, currentPromptLabel(prompts[2], lesson.title))
      }
    ];
  }
 ...
 * @param {string} prompt
 */
function buildLldPracticeTemplate(lessonTitle, structure, prompt) {
  return [
    `# ${lessonTitle}`,
    '',
    `> Prompt: ${prompt}`,
    ...
    '## Class sketch or core API',
    '```python',
    '# Sketch the key classes, methods, invariants, and a tiny main() demo or acceptance check.',
    'class Example:',
    '    def __init__(self):',
    '        ...',
    '```'
